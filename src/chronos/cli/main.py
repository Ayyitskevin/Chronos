...
def cmd_halt(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # The store refused before touching disk (its path rule); report it typed rather
...
def cmd_rearm(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # The store's own rules refused before writing anything — the note rule (a
        # non-empty operator note, for the audit trail) or the path rule — and this only
        # reports the refusal typed, before any other output: `--note "$NOTE"` with NOTE
        # unset used to print "clearing halt (...)" and then let the ValueError escape
        # main() as a traceback. The note rule keeps its operator wording; every other
        # store reason is printed in the store's own words.
        reason = str(error)
        if reason == "rearm requires a non-empty operator note":
            reason = (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    campaign = sub.add_parser("campaign", help="autonomy campaign inspection tools (read-only)")
