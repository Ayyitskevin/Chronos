...
def test_preflight_passes_without_dotenv_network_or_writes(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        "chronos.cli.main.load_config",
        lambda values: SimpleNamespace(
            provider="local", backend_url=values["CHRONOS_WORKER_BACKEND_URL"], symbols=("SPY",)
        ),
    ) if False else None
...
def test_preflight_rejects_forwarding_unit_and_never_calls_seeders(monkeypatch, tmp_path, capsys):
    (tmp_path / "worker.service").write_text(
