...
PTH=$(ls "$ROOT"/.venv/lib/python*/site-packages/__editable__*.pth | head -n 1); pth0=$(sha256sum < "$PTH")
(cd "$C10" && bash -c '. "$1"; W=$(mktemp -d); sandbox_snapshot "$W"; sandbox_run "$W" .venv/bin/python -I -c "import chronos; print(chronos.__file__)"; sandbox_run "$W" git rev-parse -q --verify refs/remotes/origin/main; rm -rf "$W"' _ "$G/lib/sandbox.sh") > "$T/r4pin.out" 2>&1
check "r4:1 PB launcher: under python -I (PYTHONPATH ignored) chronos imports from the SNAPSHOT — the lane venv's editable .pth is overlaid in the sandbox only" "sed -n 1p $T/r4pin.out | grep -qE '^/.*/snap/src/chronos/__init__\.py$'"
check "r4:1 PB launcher: the snapshot carries the lane's origin/main ref (a local fetch, as data), so base-ref tests resolve it" "[ \"\$(sed -n 2p $T/r4pin.out)\" = \"\$(git -C $C10 rev-parse refs/remotes/origin/main)\" ]"
check "r4:1 PB the lane venv's editable .pth is byte-unchanged on disk (the overlay exists only inside the sandbox)" "[ \"\$(sha256sum < $PTH)\" = \"$pth0\" ] && grep -qx '$ROOT/src' $PTH"
...
if len(sys.argv) > 1 and sys.argv[1] in ("-c", "-I"):  # the identity assert runs for real
    os.execv("$RPY", ["$RPY", *sys.argv[1:]])  # argv[0] names the venv interpreter, so it finds its venv
leaks = []
