...
| `data/platform_audit.head.json` | **Head anchor** for the audit log — the expected record count and head hash (`chronos.auditlog`, added 2026-09-12) | **It must travel with the log.** A log restored without it is BROKEN until the owner bootstraps it; an older log restored beside a newer anchor is BROKEN as a truncation/rollback. Restoring both from one older snapshot passes and is not locally detectable — the disclosed residual (R-79). |
