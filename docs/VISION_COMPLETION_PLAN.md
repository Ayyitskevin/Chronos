...
- Reproducible package/release validation: clean venv install, all migrations, static
  ...
  **Partial delivery (2026-08-27):** the CI release-artifact gate now builds and installs the
  ...
  every packaged `src/chronos/**/__main__.py` command surface. Dependency/secret/static scans, an
  installed-wheel migration upgrade drill, SBOM generation, build-backend pinning, and artifact
  signing remain open; this does not satisfy the Phase 2 exit.
