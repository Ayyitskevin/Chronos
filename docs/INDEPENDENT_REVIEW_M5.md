...
| 10 | LOW-MED | `docs/DEPLOYMENT.md` still said "there is no lockfile in this repository," contradicting SECURITY.md and the R-15 MITIGATED status; SECURITY.md's "everything is hash-checked" overclaimed (at M5, the PEP 517 build backend and pip were outside the gate). | DEPLOYMENT.md installs from hash-verified locks. The backend was subsequently added to `requirements-build.lock`; pip remains the disclosed residual. `aeventkit` was verified as `ib_async`'s genuine declared dependency (ib-api-reloaded's republication of eventkit), recorded in SECURITY.md. |
