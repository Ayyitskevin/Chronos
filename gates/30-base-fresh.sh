...
if ! base="$(gh api graphql -F owner='{owner}' -F name='{repo}' -F number="$PR_NUMBER" -f query="$query" -q .data.repository.pullRequest.baseRefOid 2>/dev/null)" \
    || [[ ! "$base" =~ ^[0-9a-f]{40}$ ]]; then
  echo "FAIL: base-fresh — could not read #$PR_NUMBER's base sha (got '${base:-}'); check gh auth/network, then re-run" >&2
  exit 1
fi
