...
if ! draft="$(gh pr view "$PR_NUMBER" --repo Ayyitskevin/Chronos --json isDraft -q .isDraft 2>/dev/null)"; then
  draft="<gh error>"
fi
case "$draft" in
  false)
  ...
  true)
    ...
    exit 1 ;;
  *)
    echo "FAIL: pr-ready — could not read #$PR_NUMBER's draft state (got '$(clean "$draft")'); check gh auth/network, then re-run" >&2
    exit 1 ;;
esac
