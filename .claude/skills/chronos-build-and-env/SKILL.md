...
| 4 | Test/lint runs leave cache dirs | `.pytest_cache/`, `.ruff_cache/`, `__pycache__/` appear in the checkout; all gitignored — `git status --porcelain` stays clean. Sandboxed containers may deny deleting them; that is harmless. |
