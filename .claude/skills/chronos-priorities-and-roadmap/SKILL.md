...
| 6 | **External-worker provenance is static and its credential is not proposal-only**: every proposal is stamped with the constant `INGRESS_IDENTITY`, and the ingress accepts the same local token every mutating route accepts. | ~~OPEN~~ **CLOSED 2026-08-12** (owner directed ADR-0023 Option A; D-24/R-48): proposer registry (`AUTONOMY_PROPOSERS_FILE`), proposal-only credential, registration-derived provenance at drain time; registry-unset keeps the static posture by design. Evidence-protocol half (per-job digest/expiry) still future ADR work. | `grep -n "proposer_id" src/chronos/supervisor/queue.py`; `.venv/bin/pytest -q tests/safety/test_proposer_credentials_exercised.py` |
...
| Suite green, 1 skip (~~~2489 passed as of 2026-08-02~~ ~~~2745~~ ~~~2767~~ ~~~2805~~ **corrected 2026-08-09: ~2889**, post Five-Tool merge, the ADR-0013 registry integration, the certified reader, and the replay-artifact capability; baseline home: `chronos-validation-and-qa` §2) | `.venv/bin/pytest -q` |
...
| Account ≈ USD 110 decision still unresolved | `grep -n "USD 110" docs/VISION_COMPLETION_PLAN.md` |
